  ...
  return (
    <div className="py-20">
          ...
          <div className="mt-20 flex flex-col gap-y-4 items-center justify-center">
            ...
            <div className="flex items-center gap-x-4 bg-[#38BDF8] py-3 px-6 rounded-[40px]">
                {tab.map((item , index)=>(
                  <button key={index} onClick={()=>setGoldtab(item)} className={`text-[18px] font-Roboto font-medium cursor-pointer transition-all duration-300 ${goldtab == item ? "bg-[#f5f5f5] py-2 px-3 rounded-4xl" : "bg-transparent py-2 px-3 rounded-4xl"}`}>{item}</button>
                ))}
            </div>
          </div>
