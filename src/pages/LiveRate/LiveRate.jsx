    ...
    <div className="py-20 px-2 md:px-0">
          ...
          <div className="mt-20 flex flex-col gap-y-4 items-center justify-center">
            ...
            <div className="flex items-center gap-x-4 bg-[#38BDF8] dark:text-black py-3 px-6 rounded-[40px]">
                {tab.map((item , index)=>(
                  <button key={index} onClick={()=>setGoldtab(item)} className={`text-[10px] md:text-[18px] cursor-pointer transition-all duration-300 ${goldtab == item ? "bg-[#f5f5f5] py-2 px-1 md:px-3 rounded-4xl" : "bg-transparent py-2 px-1 md:px-3 rounded-4xl"}`}>{item}</button>
                ))}
            </div>
          </div>
