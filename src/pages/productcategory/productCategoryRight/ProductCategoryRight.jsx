...
const ProductCategoryRight = ({categoryid}) => {
  
   const findcategory = product.find((item)=> item.id === categoryid);
  ...
  const showingTo = Math.min(endindex, product.length); 
  ...
  return (
    <div className='w-full  px-6 py-4'>
      ...
      <div className='flex items-start justify-between'>
         <div>
            <p className='text-[18px] text-[#cda6a6] font-Roboto font-medium'>{`${showingFrom} - ${showingTo}`} Products Of {`${product.length}`} Products</p>
         </div>
         <div className='flex items-center gap-x-4'>
            ...
            <div className='flex items-center gap-x-4 text-[#cda6a6]'>
      ...
      <nav className='flex items-center justify-center mt-20 rounded' aria-label="Page navigation example">
        <ul className="flex -space-x-px text-sm">
          ...
          {[...new Array(Math.ceil(productlength || 4))].map((_ , index)=>(
            <li>
